...
def accuracy(output, target):
    ...
    denominator = (output[:]**2 + target**2).sum((1,2))
    loss = (intersection + eps)/(denominator+eps)
    return loss.mean()

    #out = output[:,0].reshape(-1)
    #target = target.reshape(-1)
    #intersection = (out * target).sum()
    #dsc = (2. * intersection + eps) / (out.sum() + target.sum() + eps)
    #return dsc

def accuracy_large(output, target):
    l,_,_,_ = output.shape
    #output = F.softmax(output[:l//2], dim=1)
    #intersection = (output[:,1] * target).sum()
    #denominator = (output[:,1] + target).sum()
    #loss = (2.0*intersection + eps)/(denominator+eps)
...
def accuracy_small(output, target):
    l,_,_,_ = output.shape
    #output = F.softmax(output[l//2:], dim=1)
    #intersection = (output[:,1] * target).sum()
    #denominator = (output[:,1] + target).sum()
    ...
    prob_larger = F.softmax(output[:l//2], dim=1)
    prob_smaller = F.softmax(output[l//2:], dim=1)
    intersection = (prob_smaller[:,1] * target).sum()
    denominator = (prob_smaller[:,1] + target).sum()
    loss = (2.0*intersection + eps)/(denominator+eps)
    return loss
...
class Accuracy():
    ...
    def __call__(self, out, yb, *args):
        if Accuracy.precision_list is not None: return Accuracy.precision_list, Accuracy.recall_list
        
        out = torch.argmax(out, dim=1)
        out = out.cpu().numpy()
        yb = yb.cpu().numpy()
        p,r,f1,_ = precision_recall_fscore_support(out.reshape(-1),yb.reshape(-1))
        Accuracy.precision_list = p
        Accuracy.recall_list = r

        return Accuracy.precision_list, Accuracy.recall_list

# precision
class Accuracy0(Accuracy):
    def __call__(self, out, yb, *args): 
        p,r = super().__call__(out, yb)
        return p[0]
# recall
class Accuracy1(Accuracy):
    def __call__(self, out, yb, *args):
        p,r = super().__call__(out, yb) 
        return r[0]

# precision
class Accuracy2(Accuracy):
    def __call__(self, out, yb, *args):
        p,r = super().__call__(out, yb)
        return p[1]
# recall
class Accuracy3(Accuracy):
    def __init__(self,is_last_metric=True):
        self.is_last_metric = is_last_metric

    def __call__(self, out, yb, *args):
        p,r = super().__call__(out, yb)
        if self.is_last_metric:
           Accuracy.precision_list = None
        return r[1]
...
class lossScheduler(CallBacks):
    def after_epoch(self):
        ...
        #self.learn.loss_func = AdjointDiceLoss(min(4*(x**2),1))
        self.learn.loss_func = AdjointDiceLoss(alpha=np.e**x-1) 
...
class Stats():
    ...
    def accumulate(self, run):
        ...
        self.tol_loss += batch_size * run.loss.cuda()
...
class SaveModelCallback(CallBacks):
    def __init__(self,name,save_dir="/scratch/un270/model/Unet/"):
